...
class ContractsControllerTest < ActionController::TestCase
  ...
  test "show view without 'edit contracts' or 'delete contracts' permissions" do
    ...
    assert_tag :tag => "a", :content => "Time Entries"
  ...
  test "show view with 'edit contracts' permission" do
    ...
    get :show, :project_id => @project.id, :id => @contract.id
    assert_tag :tag => "a", :attributes => { :title => "Edit" }
    assert_tag :tag => "a", :content => "Bulk Assign Time Entries"
  end
  ...
  test "show view without 'edit time entries' permission" do
    Role.find(4).add_permission! :view_contract_details
    @time_entry.contract_id = @contract.id
    @time_entry.save
    get :show, :project_id => @project.id, :id => @contract.id
    assert !assigns(:time_entries).empty?
    assert_no_tag :tag => "a", :attributes => { :href => "/time_entries/#{@time_entry.id}/edit" }, :content => "Edit"
  end
